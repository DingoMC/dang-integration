"""dang$$ BoulderingScore ORM

This module contains BoulderingScore Skarpa Model
Created by: DingoMC
Cores: akka, umbry
...
class BoulderingScore:
    @staticmethod
    def select (filter: list[str] = None, where: dict = None, join: list[str] = None, order_by: dict = None):
        sql_query = 'SELECT ' + ConstructFilter(filter) + ' FROM "BoulderingScore"'
        if join is not None:
            sql_query += ' blds'
            if 'Competition' in join:
                sql_query += ' LEFT JOIN "Competition" c ON c.id = blds.competition_id'
                if 'Season' in join:
    ...
    def insert (fields: list[str], values: list[list[str]]):
        sql_query = 'INSERT INTO "BoulderingScore" ' + ConstructInsert(fields, values) + ' RETURNING id'
        result = ExecuteSkarpaSQL(sql_query)
        return result
    
    @staticmethod
    def updateRow (fields: dict, where: dict = None):
        sql_query = 'UPDATE "BoulderingScore" SET '
    ...
    def count (where: dict = None, join: list[str] = None):
        sql_query = 'SELECT COUNT(*) FROM "BoulderingScore"'
        if join is not None:
            sql_query += ' blds'
        sql_query += ConstructWhere(where)
        result = ExecuteSkarpaSQL(sql_query)
    ...
    def upsert (updateFields: dict, insertFields: list[str], insertValues: list[str], onConflict: list[str]):
        sql_query = 'INSERT INTO "BoulderingScore" ' + ConstructUpsert(updateFields, insertFields, insertValues, onConflict)
        ExecuteSkarpaSQLUpdate(sql_query)
